<template>
  <div id="index">
	...
	<tr id="titulo-tabela">
      ...
      <tr class="dados-tabela" v-for="transacaot of transacao" :key="transacaot.id">
        <td>{{ transacaot.data }}</td>
        <td>{{ transacaot.descricao }}</td>
        <template class="dados-tabela" v-if="transacaot.name == 'Saque'">
            <td style="color: #f00;"> {{ formatPrice(transacaot.amount) }}</td>
        </template>
        <template class="dados-tabela" v-else>
        <td>{{ formatPrice(transacaot.amount) }}</td>
        </template>
...
export default {
  ...
  mounted() {
    api.get('/transacoes').then(reponse => {
      this.transacao = reponse.data;
    });
